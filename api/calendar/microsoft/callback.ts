 ...
 * Maneja la respuesta de Microsoft después de la autorización
 */

import { exchangeCodeForTokens } from '../../../server/_core/calendar/oauth-microsoft.js';
...
export default async function handler(request: Request) {
  try {
    ...
    if (error) {
      console.log('❌ User cancelled Microsoft Calendar authorization:', error);
    ...
    console.log('✅ Received Microsoft Calendar callback for userId:', userId);
    ...
    return Response.redirect(
      `${process.env.APP_URL || 'https://www.pianoemotion.com'}/settings/calendar-settings?success=microsoft`,
      302
    );
  } catch (error) {
